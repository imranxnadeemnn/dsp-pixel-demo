...
'    for(var i=0;i<TRACK.length;i++){ var v=qp(TRACK[i]); if(v){ urlAttr[TRACK[i]]=v; if(TRACK[i]==="click_id") hasClick=true; } }',
'    if(hasClick){ for(var k in urlAttr){ attr[k]=urlAttr[k]; } if(!attr.ft) attr.ft=Date.now(); setCookie(COOKIE, JSON.stringify(attr), DAYS); }',
